...
class DiscoverEntity:
    """Create an entity if the Ramses id is known, otherwise create them on device discovery"""
    ...
    def _add_discovered_sensors(self) -> None:
        ...
        if not self.ramses_id:
            return
        _LOGGER.debug(f"Creating discovered {self.label} sensors ({self.ramses_id})")
        self._add_sensors()

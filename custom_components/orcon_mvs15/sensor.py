...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    rssi_sensor = SignalStrengthSensor(fan_id, pull_coordinator)
    async_add_entities([hum_sensor, rssi_sensor])
    if (
...
class SignalStrengthSensor(CoordinatorEntity, SensorEntity):
    def __init__(self, fan_id, coordinator):
        super().__init__(coordinator)
        self.coordinator = coordinator
        self._attr_name = "Orcon MVS-15 RSSI"
        self._attr_native_unit_of_measurement = SIGNAL_STRENGTH_DECIBELS_MILLIWATT
        self._attr_device_class = SensorDeviceClass.SIGNAL_STRENGTH
        self._attr_state_class = SensorStateClass.MEASUREMENT
        self._attr_unique_id = f"orcon_mvs15_rssi_{fan_id}"
        self._attr_device_info = DeviceInfo(identifiers={(DOMAIN, fan_id)})
    ...
    def native_value(self):
        if not self.coordinator.data:
            return None
        return self.coordinator.data.get("fan_rssi")
